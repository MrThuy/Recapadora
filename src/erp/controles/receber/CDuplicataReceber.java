 ...
 * @author Cliente
 */
public class CDuplicataReceber {
...
//        }
    ...
    private void carregaGrid(JInternalFrame frame) {
        itens = CTituloReceber.listaTitulosRecebePorCliente(CTituloReceber.TIPO.TODOS, cliente.getIdcorrentista(), new String[]{"correntista","tipoTitulo", "idfinanceira"});
        ((TituloReceberTableModel) ((JTable) ERPFrames.getComponent(frame, "jTable1")).getModel()).setLinhas(itens);
    }
    ...
    public void duplicataReceber(int idcliente, int idtipotitulo, int numero, int parcela) {
        duplicataReceber(CTituloReceber.procuraTituloReceber(idcliente, numero, parcela, idtipotitulo));
    }

    public void duplicataReceber(TituloReceber titulo) {
        InputStream inputStream = getClass().getResourceAsStream("/erp/relatorios/DuplicataReceber.jasper");
        ...
        ImageIcon gto = new ImageIcon(getClass().getResource("/recursos/Pneu74.jpg"));
        ...
        try {
            ...
            ERPRelatorios.openReport("Duplicata", inputStream, parametros,
                    ds);
